...
class WorkThread {
 public:
  ...
  auto Run(Func &&f, Args &&...args) -> std::future<
  ...
  std::thread *operator->() { return &operator*(); }

  std::thread const *operator->() const { return &operator*(); }

  auto get_id() const { return thread_.get_id(); }
